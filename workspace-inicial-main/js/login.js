...
document.getElementById('formulario').addEventListener('submit', function (e) {
  ...
  */
  ...
  fetch("http://localhost:3000/login", {
    ...
    .then((data) => {
      // Obtener el token desde la respuesta del servidor
      const token = data.token;

      // Guardar el token (por ejemplo, en localStorage o sessionStorage)
      localStorage.setItem("token", token);
      if (status === 200) {
        window.location.href = "/index.html";
      }
      // Hacer otras cosas después de obtener el token
    })
    .catch((error) => console.error("Error en la solicitud de inicio de sesión:", error));
  //});
});
